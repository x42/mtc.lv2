...
#   make CFLAGS=-O2
#   make install DESTDIR=$(CURDIR)/debian/mtc.lv2 PREFIX=/usr
#
...
mtc_VERSION?=$(shell git describe --tags HEAD 2>/dev/null | sed 's/-g.*$$//;s/^v//' || echo "LV2")
###############################################################################
LIB_EXT=.so
...
ifneq ($(MOD),)
  ...
  MODLABEL=mod:label \"MIDI Timecode\";
...
override CFLAGS += -fPIC -std=c99 -fvisibility=hidden
...
$(BUILDDIR)$(LV2NAME)$(LIB_EXT): src/$(LV2NAME).c libtimecode/timecode.c
	@mkdir -p $(BUILDDIR)
	$(CC) $(CPPFLAGS) $(CFLAGS) \
	  -o $(BUILDDIR)$(LV2NAME)$(LIB_EXT) \
		src/$(LV2NAME).c libtimecode/timecode.c \
